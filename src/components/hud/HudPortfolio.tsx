    ...
    <>
      ...
      <div className="content-scroll">
        <div className="page-shell">
          {/* HERO */}
          <section className="section" style={{ minHeight: 'calc(100vh - 96px)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-5 w-full">
              ...
              <div className="flex gap-4">
                <a href="https://github.com/SoyNani" target="_blank" rel="noreferrer" className="text-[#8a8a94] hover:text-white transition-colors" aria-label="GitHub">
                  <Github size={18} strokeWidth={1.5} />
                </a>
                <a href="https://www.linkedin.com/in/laura-daniela-lópez-jiménez" target="_blank" rel="noreferrer" className="text-[#8a8a94] hover:text-white transition-colors" aria-label="LinkedIn">
          ...
          <section className="section" id="about">
            <div className="grid md:grid-cols-12 gap-10 md:gap-12">
              <div className="md:col-span-4">
                ...
                <h2 className="mt-4 text-3xl md:text-4xl font-semibold tracking-tight leading-tight text-white" style={sans}>
                  Mentalidad de
                  <br />
              ...
              <div className="md:col-span-8 space-y-5">
                <p className="text-[#a8b0c4] font-light leading-relaxed text-base md:text-lg" style={sans}>
                ...
                <div className="grid sm:grid-cols-2 gap-3 pt-2">
                  {[
                    { t: 'Full-stack', d: 'Del UI al cloud sin perder visión de negocio.', accent: 'border-l-[#5b8cff]' },
                    { t: 'Centrada en humanos', d: 'UX, comunicación y colaboración en el proceso.', accent: 'border-l-[#ff6bb5]' },
                    { t: 'Ownership', d: 'Decisiones técnicas alineadas a producto.', accent: 'border-l-[#9b6dff]' },
                    { t: 'Aprendizaje', d: 'Certificaciones y retos como hábito.', accent: 'border-l-[#ff9f43]' },
                  ].map((item) => (
                    <div
                      key={item.t}
                    ...
                    >
                      ...
                      <p className="mt-1.5 text-xs text-[#a8b0c4] font-light leading-relaxed">{item.d}</p>
                    </div>
                  ))}
                </div>
          ...
          <section className="section" id="work">
            ...
            <h2 className="mt-3 text-3xl md:text-4xl font-semibold tracking-tight text-white" style={sans}>
              Experiencia laboral
            </h2>

            <div className="mt-10 space-y-4">
              {protectedContent.experience.map((exp, i) => {
                const accents = ['#ff6bb5', '#9b6dff', '#5b8cff', '#ff9f43']
                ...
                return (
                  <motion.article
                    ...
                    className="card-glow rounded-2xl p-5 md:p-7"
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                      <div>
                        ...
                        <h3 className="text-lg md:text-xl font-semibold text-white mt-1" style={sans}>
                          {exp.role}
                        </h3>
                        <p className="text-sm font-medium mt-0.5" style={{ color }}>
                          {exp.company}
                        </p>
                        <p className="text-xs font-mono text-[#6a6a78] mt-1">
                          {exp.period} · {exp.location}
                        </p>
                      </div>
                      {exp.isActual && (
                        <span
                          className="self-start text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-full text-black font-semibold"
                    ...
                    <ul className="mt-4 space-y-2">
                      {exp.achievements.map((a) => (
                        <li
                          key={a.slice(0, 32)}
                          className="text-sm text-[#a8b0c4] font-light leading-relaxed flex gap-2"
                          style={sans}
                        >
            ...
            <div className="mt-12">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {protectedContent.education.map((ed) => (
                  <div key={ed.degree + ed.institution} className="card-glow rounded-xl px-4 py-4">
                    <p className="text-sm font-medium text-white">{ed.degree}</p>
                    <p className="text-xs text-[#a8b0c4] mt-1">{ed.institution}</p>
                    <p className="text-[10px] font-mono text-[#6a6a78] mt-2 uppercase">{ed.period}</p>
                  </div>
                ))}
              </div>
              {protectedContent.recognition && (
                <div
                  className="mt-4 rounded-2xl p-5 md:p-6 card-glow"
          ...
          <section className="section" id="projects">
            ...
            <h2 className="mt-3 text-3xl md:text-4xl font-semibold tracking-tight mb-10 text-white" style={sans}>
              Proyectos
            </h2>
            ...
            <div className="space-y-6">
              {projects.map((p, i) => (
                ...
                >
                  ...
                  <div className={`md:col-span-7 p-5 md:p-8 ${i % 2 === 1 ? 'md:order-1' : ''}`}>
                    <span className="font-mono text-[10px] tracking-widest accent-purple">0{i + 1}</span>
                    <h3 className="mt-1 text-xl md:text-2xl font-semibold text-white">{p.title}</h3>
                    <p className="mt-2 text-sm text-[#a8b0c4] font-light leading-relaxed">{p.description}</p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {p.tags.map((t, ti) => {
                        const chips = ['chip-pink', 'chip-purple', 'chip-blue']
                        return (
                          <span key={t} className={`text-[10px] px-2 py-0.5 rounded border ${chips[ti % 3]}`}>
                            {t}
                          </span>
                        )
                      })}
                    ...
                    <div className="mt-4 flex gap-4">
                      {p.demoUrl && (
                        <a href={p.demoUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm accent-blue hover:opacity-80">
                          Demo <ExternalLink size={13} />
                        </a>
                      )}
                      {p.githubUrl && (
                        <a href={p.githubUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm text-[#6a6a78] hover:text-white">
          ...
          {/* CONTACT */}
          <section className="section" id="contact" style={{ minHeight: '45vh', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
            <div className="w-full max-w-xl">
              <p className="font-mono text-[10px] tracking-[0.25em] uppercase accent-pink">
                05 // Señal
              </p>
              <h2 className="mt-4 text-[clamp(2rem,6vw,3.5rem)] font-semibold tracking-tight text-white leading-tight" style={sans}>
                Construyamos algo
                <br />
                <span className="bg-gradient-to-r from-[#ff6bb5] via-[#9b6dff] to-[#5b8cff] bg-clip-text text-transparent">
                  juntos
